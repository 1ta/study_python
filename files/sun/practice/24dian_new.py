...
def operate(a,op,b):
    return ops_dic[op](a,b)

def result_24_1(num, op):
    result = operate(num[0], op[0], num[1])
    if  not divise(num[0],num[1]) is None:
        result = operate(result, op[1], num[2])
        if not divise(result,num[2]) is None:
...
def result_24_2(num, op):
    ...
    if not divise(num[0],num[1]) is None and not divise(num[2],num[3]) is None:
